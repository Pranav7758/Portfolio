  ...
  return (
    ...
    >
      <div className="container mx-auto flex justify-between items-center">
        ...
        >
          <span className="bg-black text-white px-2 py-1 rounded mr-1">PB</span>
          <span>Pranav</span>
        </motion.div>

        <ul className="hidden lg:flex items-center gap-x-7 font-semibold">
          {["about", "skills", "projects", "myart", "education", "certificates", "contact"].map((section) => (
            <motion.li
              key={section}
              className="group"
              whileHover={{ scale: 1.1 }}
            >
              <button onClick={() => scrollToSection(section)}>
                {section === "myart" ? "My Art" : section.charAt(0).toUpperCase() + section.slice(1)}
              </button>
              <motion.span
